...
contract ConvexStrategy is BaseUpgradeableStrategy {
    ...
    function enterRewardPool() internal {
        ...
        IERC20(_underlying).forceApprove(booster, 0);
        IERC20(_underlying).forceApprove(booster, entireBalance);
    ...
    function _liquidateReward() internal {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            if (rewardBalanceAmount == 0 || token == _rewardToken) {
                continue;
            }
    ...
    function depositCurve() internal {
        ...
        if (_depositToken != weth) {
            IERC20(_depositToken).forceApprove(_curveDeposit, 0);
            IERC20(_depositToken).forceApprove(_curveDeposit, tokenBalance);
        }
